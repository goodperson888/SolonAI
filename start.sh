...
# 后端依赖
echo -e "  📦 检查后端依赖..."
pip3 install -q fastapi uvicorn python-dotenv pydantic pydantic-settings httpx 2>/dev/null || true

# AI 服务依赖
echo -e "  📦 检查AI服务依赖..."
pip3 install -q langchain langchain-openai langgraph openai 2>/dev/null || true
...
PYTHONPATH="$ROOT_DIR/services/ai-agents:$PYTHONPATH" python3 -m uvicorn main:app --host 0.0.0.0 --port 8000 --reload &
API_PID=$!
